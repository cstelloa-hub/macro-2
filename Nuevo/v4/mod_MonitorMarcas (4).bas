...
'==================================================================
' HOJA RESUMEN
'   1. Detalle de marcas de la semana
'   2. Cuadro por fondo, partido en LOCALES y EXTRANJEROS
'   3. Por estrategia, dentro de cada mitad
'   4. Por moneda      5. Por vintage
'   6. Top winners y top losers
'==================================================================
...
Private Function ImpactaAlgo(i As Long) As Boolean
    ImpactaAlgo = (evBas(i, 1, 1) <> 0 Or evBas(i, 2, 1) <> 0 Or evBas(i, 3, 1) <> 0)
End Function
...
Private Sub ConstruirResumen()
    ...
    r = DetalleSemana(ws, 5, lun, dom)

    r = CuadroFondos(ws, r + 3, "2.  RESUMEN POR FONDO - LOCALES", 1, _
                     lun, dom, mIni, pIni, aIni)
    r = CuadroFondos(ws, r + 2, "     RESUMEN POR FONDO - EXTRANJEROS", 2, _
                     lun, dom, mIni, pIni, aIni)

    r = Desglose(ws, r + 3, "3.  LOCALES - POR ESTRATEGIA", 1, 1, _
                 lun, dom, mIni, pIni, aIni)
    r = Desglose(ws, r + 2, "     EXTRANJEROS - POR ESTRATEGIA", 1, 2, _
                 lun, dom, mIni, pIni, aIni)

    r = Desglose(ws, r + 3, "4.  POR MONEDA", 4, 0, lun, dom, mIni, pIni, aIni)
    r = Desglose(ws, r + 3, "5.  POR VINTAGE", 3, 0, lun, dom, mIni, pIni, aIni)
...
'------------------------------------------------- 1. detalle
Private Function DetalleSemana(ws As Worksheet, r0 As Long, _
                               lun As Double, dom As Double) As Long
    Dim i As Long, f As Long, r As Long, n As Long
    Dim gI As Double, gR As Double, gH As Double
    Dim t1 As Double, t2 As Double, t3 As Double, tt As Double
    Dim sI As Double, sR As Double, sH As Double

    ws.Cells(r0, 2).Value = "1.  MARCAS DE LA SEMANA   (contribucion en pb)"
    ws.Cells(r0, 2).Font.Bold = True
    ws.Range(ws.Cells(r0 + 1, 2), ws.Cells(r0 + 1, 13)).Value = _
        Array("Dia", "Fondo alternativo", "Var Adj", "pb F1", "pb F2", "pb F3", _
              "Total", "vs INF", "vs RIF", "vs HAF", "Exposicion", "Estrategia")
    ...
    For i = 1 To nEv
        If evFec(i) >= lun And evFec(i) <= dom Then
            gI = 0: gR = 0: gH = 0
            For f = 1 To 3
                gI = gI + evBas(i, f, 2) * pbF
                gR = gR + evBas(i, f, 3) * pbF
                gH = gH + evBas(i, f, 4) * pbF
            Next f
            n = n + 1
            ...
            ws.Cells(r, 9).Value = gI
            ws.Cells(r, 10).Value = gR
            ws.Cells(r, 11).Value = gH
            ws.Cells(r, 12).Value = evExp(i)
            ws.Cells(r, 13).Value = evEst(i)
            t1 = t1 + evBas(i, 1, 1) * pbF
            t2 = t2 + evBas(i, 2, 1) * pbF
            t3 = t3 + evBas(i, 3, 1) * pbF
            tt = tt + TotalPRF(i)
            sI = sI + gI: sR = sR + gR: sH = sH + gH
            r = r + 1
        End If
    Next i
    ...
    ws.Cells(r, 9).Value = sI
    ws.Cells(r, 10).Value = sR
    ws.Cells(r, 11).Value = sH
    ws.Range(ws.Cells(r, 2), ws.Cells(r, 13)).Font.Bold = True

    ws.Range(ws.Cells(r0 + 2, 2), ws.Cells(r - 1, 2)).NumberFormat = "dd/mm/yyyy"
    ws.Range(ws.Cells(r0 + 2, 4), ws.Cells(r, 4)).NumberFormat = "0.00%"
    ws.Range(ws.Cells(r0 + 2, 5), ws.Cells(r, 11)).NumberFormat = "#,##0.00"
    FormatoTabla ws, r0 + 1, 2, r, 13
    DetalleSemana = r
End Function
...
Private Function CuadroFondos(ws As Worksheet, r0 As Long, titulo As String, ambito As Long, _
    ...
    Dim gS(1 To 4) As Double, gM(1 To 4) As Double
    Dim gP(1 To 4) As Double, gY(1 To 4) As Double
    Dim tS As Long, tM As Long, tP As Long, tY As Long

    ws.Cells(r0, 2).Value = titulo
    ws.Cells(r0, 2).Font.Bold = True

    ws.Cells(r0 + 1, 4).Value = "SEMANA"
    ws.Cells(r0 + 1, 9).Value = "MTD"
    ws.Cells(r0 + 1, 14).Value = "PERIODO"
    ws.Cells(r0 + 1, 19).Value = "YTD"
    ws.Range(ws.Cells(r0 + 1, 4), ws.Cells(r0 + 1, 19)).Font.Bold = True

    ws.Range(ws.Cells(r0 + 2, 2), ws.Cells(r0 + 2, 22)).Value = Array("Fondo", _
    ...
    For f = 1 To 3
        ...
        For k = 1 To 4
            gS(k) = gS(k) + sS(k): gM(k) = gM(k) + sM(k)
            gP(k) = gP(k) + sP(k): gY(k) = gY(k) + sY(k)
        Next k
        r = r + 1
    Next f

    ' conteo de marcas distintas del ambito (una marca en F2 y F3 cuenta una vez)
    For i = 1 To nEv
        d = evFec(i)
        If d > 0 And d <= dom And EnAmbito(i, ambito) And ImpactaAlgo(i) Then
            If d >= aIni Then tY = tY + 1
            If d >= pIni Then tP = tP + 1
            If d >= mIni Then tM = tM + 1
            If d >= lun Then tS = tS + 1
        End If
    Next i

    ws.Cells(r, 2).Value = "Total"
    ws.Cells(r, 3).Value = tS
    For k = 1 To 4: ws.Cells(r, 3 + k).Value = gS(k): Next k
    ws.Cells(r, 8).Value = tM
    For k = 1 To 4: ws.Cells(r, 8 + k).Value = gM(k): Next k
    ws.Cells(r, 13).Value = tP
    For k = 1 To 4: ws.Cells(r, 13 + k).Value = gP(k): Next k
    ws.Cells(r, 18).Value = tY
    For k = 1 To 4: ws.Cells(r, 18 + k).Value = gY(k): Next k
    ws.Range(ws.Cells(r, 2), ws.Cells(r, 22)).Font.Bold = True

    ws.Range(ws.Cells(r0 + 3, 4), ws.Cells(r, 7)).NumberFormat = "#,##0.00"
    ws.Range(ws.Cells(r0 + 3, 9), ws.Cells(r, 12)).NumberFormat = "#,##0.00"
    ws.Range(ws.Cells(r0 + 3, 14), ws.Cells(r, 17)).NumberFormat = "#,##0.00"
    ws.Range(ws.Cells(r0 + 3, 19), ws.Cells(r, 22)).NumberFormat = "#,##0.00"
    FormatoTabla ws, r0 + 2, 2, r, 22
    CuadroFondos = r
End Function
...
Private Sub Ranking(ws As Worksheet, r0 As Long, lun As Double, dom As Double, winners As Boolean)
    ...
    ws.Cells(r0, 2).Value = IIf(winners, "6.  TOP WINNERS de la semana", "     TOP LOSERS de la semana")
    ws.Cells(r0, 2).Font.Bold = True
