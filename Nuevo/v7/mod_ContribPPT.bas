...
Private Sub ConstruirCuadros()
    ...
    '---- que filas entran: lo que marco dentro de la ventana mostrada ----
    Dim db As Object, ds As Object, dn As Object
    ...
    For i = 1 To nR
        If R(i).Dia <= corte And (Year(CDate(R(i).Dia)) = anio Or R(i).Dia > fyBase) Then
            ...
            If Not dn.Exists(R(i).Bloque & "|" & R(i).Subgrupo & "|" & R(i).Nombre) Then _
                dn.Add R(i).Bloque & "|" & R(i).Subgrupo & "|" & R(i).Nombre, 1
        End If
    Next i
...
Private Sub PintarHoja(ws As Worksheet, filas As Collection, finM() As Double, basM() As Double, _
    ...
    Dim f As Variant, niv As Long, sel As Variant, u As Range
    Dim frm(1 To 1, 1 To 16) As Variant
    Dim hijos As String

    sel = ws.Range("C2").Value
    If Not IsNumeric(sel) Then sel = 2
    If sel < 1 Or sel > 3 Then sel = 2
